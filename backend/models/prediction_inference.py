...
# Look up prepared model predictions from the shared predictions CSVs.
class PredictionInference:
    # This helper keeps route guidance deterministic by reading from prepared predictions.
    def __init__(self, artifacts: PredictionArtifacts | None = None):
        self.artifacts = artifacts or load_prediction_artifacts()
        # We cache by dataset year because route guidance may switch between 2006 and 2014 in one session.
        self._predictions_cache: dict[str, pd.DataFrame] = {}
        self._route_guidance_cache: dict[str, pd.DataFrame] = {}

    @cached_property
    # Cache the default predictions table because 2014 is the most common UI dataset.
    def default_predictions_df(self) -> pd.DataFrame:
        return self.artifacts.predictions.copy()

    # Load or reuse the prepared predictions table for the requested dataset year.
    def get_predictions_df(self, data_key: str = "2014") -> pd.DataFrame:
        ...
        if normalized not in self._predictions_cache:
            # Keep 2014 warm because the UI defaults there, but still allow 2006 lookup on demand.
            if normalized == "2014":
    ...
    # Keep only the October rows because the route-guidance UI is built around that month.
    def get_route_guidance_predictions_df(self, data_key: str = "2014") -> pd.DataFrame:
        ...
        if normalized not in self._route_guidance_cache:
            ...
            # Route guidance currently focuses on October so the available date picker, graph snapshots,
            # and prepared route examples all refer to the same seasonal window.
    ...
    @cached_property
    # Use a site-level 75th percentile of actual flow as a stable congestion reference.
    def default_site_reference_flows(self) -> dict[str, float]:
        # The reference flow is not a model output.
        # It is a historical baseline used only to interpret how congested a predicted flow looks.
        site_actuals = (
    ...
    # Build the site-level reference flows for the requested dataset.
    def get_site_reference_flows(self, data_key: str = "2014") -> dict[str, float]:
        normalized = data_key.strip().lower()
        if normalized == "2014":
            return self.default_site_reference_flows

        # Non-default datasets reuse the same baseline idea, but we compute it on demand
        # so the common 2014 path stays cached and lightweight.
    ...
    # Return valid date and time options derived from the prepared predictions.
    def get_time_options(self, data_key: str = "2014") -> dict[str, object]:
        ...
        # Defaults are adjusted to a valid value from the file so the UI never loads an impossible date/time pair.
    ...
    # Resolve one timestamp exactly to avoid mixing predictions from different intervals.
    def resolve_target_timestamp(self, data_key: str = "2014", target_datetime: str | None = None) -> pd.Timestamp:
        ...
        # Route guidance uses exact prepared timestamps so models are compared on the same interval.
    ...
    # Aggregate movement-level predictions into one site-level flow map for routing.
    def predict_site_flow_map(
        ...
        # The CSV stores movement-level predictions, but routing works at the site level.
        # We sum all movements for the same SCATS site to get one site-level flow proxy.
        ...
        # String keys keep the output aligned with the graph node ids used everywhere else in the backend.
