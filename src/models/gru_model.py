"""GRU training module for SCATS traffic forecasting.

This module provides typed reusable utilities and keeps all training behavior
inside explicit function calls to avoid import-time side effects.
"""
...
def build_gru_model(
  ...
  """Build and compile the GRU forecasting model.
...
def plot_training_curve(history: History, output_path: Path, title: str) -> None:
...
def train_gru_model(
  ...
  """Train a GRU model using project defaults or caller-provided overrides.
  ...
  Returns:
    The trained GRU model with best weights restored.
  """
  ...
  callbacks = create_training_callbacks(
    model_output_path=model_dir / "gru_model.keras",
...
def main() -> None:
  """Train the GRU model when the module is run as a script."""
